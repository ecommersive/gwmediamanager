              ...
              if (currentData === 'Playlist' || currentData === 'Ads') {
                ...
                return (
                  <tr key={index} style={{ backgroundColor: index % 2 === 0 ? 'transparent' : '#f0f0f0' }}>
                    ...
                    {isAdmin && <td><button className='action-button' onClick={() => { setShowModal(true); setFileName(item.FileName); setNotes(item.notes); setCatData('Comments'); setMode('configureData');setfolderViewNum(item.folder)  }}>Comments</button></td>}
                  </tr>
                );
              }
            })}
